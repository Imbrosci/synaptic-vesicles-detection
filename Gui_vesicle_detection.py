...
class VesAnalysisGui(tk.Frame):
    ...
    def graphic_results(self):
        ...
        # delete old windows / images, if any, and create a new one
        try:
            self.canvas1.delete('all')
        except Exception:
            self.canvas1 = tk.Canvas(self.master, width=800, height=600,
    ...
    def sliding_detection(self):
        ...
        # loop through image.shape[1] steps = sliding_size
        for x in range(0, np_img.shape[1], sliding_size):
            self.var.set('Processing image number: ' + str(self.counter))

        # loop through image.shape[0] steps = sliding_size
            for y in range(0, np_img.shape[0], sliding_size):
                ...
                if preds == 1:
                    ...
                    pvalue = np.exp(valuemax)/(np.exp(valuemax) +
                                               np.exp(valuemin))
        ...
        # loop through the found objects (labelarray)
        ...
        # loop through the detected vesicles
            ...
            for i in range(len(x_labels_semifinal)):
                ...
                for o in range(7):
                    for v in range(7):
                        ...
                        for rv in range(6):
                            for rh in range(6):
                                ring = self.drawing_ellipse_circle(7+rv, 7+rh)
                                if (np.abs(rv-rh) < 5):
                                    matrix = ring*snap
                                    st_dev = np.std(
    ...
    def manual_correction1(self):
        ...
        # delete old windows / images, if any, and create a new window
        try:
            self.canvas1.delete('all')
        except Exception:
            self.canvas1 = tk.Canvas(self.master, width=800, height=600,
    ...
    def excel_update2(self):
        ...
        # if the result file has the estimation of the vesicles area
            ...
            img_corr = self.img_to_correct.resize((
                int(self.img_to_correct.size[0]*self.pixel_size_final/2.27),
                int(self.img_to_correct.size[1]*self.pixel_size_final/2.27)))
