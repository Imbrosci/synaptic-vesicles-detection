...
class VesAnalysisGui(tk.Frame):
    ...
    def init_window(self):
        ...
        run_analysis.add_command(label="Vesicles detection", command=self.start_analysis1)
    ...
    def start_analysis1(self):
        ...
        label3 = tk.Label(self.master, text='Do you want the estimation of the vesicles area? (y/n)')
        ...
        self.canvas1.create_window(400, 335, window=self.ves_area_analysis)
        ...
        button1=tk.Button(self.master,text='Search directory',command=self.start_analysis2,font=('Helvetica', '10'))
    ...
    def image_plus_results2(self):
        ...
        self.toolbar=tkagg.NavigationToolbar2Tk(self.canvas2, self.master)
        self.toolbar.update()
    ...
    def graphic_results(self):
        ...
        label1 = tk.Label(self.master, text='Select the directory where all the results files are located')
        label1.config(font=('helvetica', 12))
        self.canvas1.create_window(400, 200, window=label1) 
    ...
    def plot(self):
        ...
        for exp_name in result_filename:
            ...
            for j in range(len(ves_count[i])):
                ...
                if (len(average_area)>0) and (len(average_area)==len(average_min_distance)):
    ...
    def sliding_detection(self):
        ...
        # setting threshold for proc_map (below 20% of 255, pixel=0)
        ...
        if ves_area_analysis =='n':
            x_labels_final=list(np.array(x_labels_semifinal)/self.pixel_size_final*2.27)
            y_labels_final=list(np.array(y_labels_semifinal)/self.pixel_size_final*2.27)
            coordinates=list(zip(x_labels_final,y_labels_final)) 
            self.coordinates=coordinates
            
        else:
            # here the area of the vesicles is calculated and masks are generated if this analysis is requested by the user
    ...
    def manual_correction2(self):
        ...
        self.new_values=[]
        self.canvas2.mpl_connect('key_press_event',self.add_or_remove_points)
    ...
    def excel_update1(self,event):
        ...
        if (len(self.new_values) >0) and (event.key == 'u'):    
            ...
            label1 = tk.Label(self.master, text='Please, remind me the pixel size (1 side!) in nanometer')
            label1.config(font=('helvetica', 12))
    ...
    def excel_update2(self):
        ...
        #calculate again distance nearest vesicles 
        ...
        if ves_area_analysis =='n':
            ...
            x_labels_final=list(x_sf)
            y_labels_final=list(y_sf)
            
        else:
            # here the area of the vesicles is re-calculated and masks are re-generated if this analysis is present in the excel file containing the results
        ...
        if len(x)!=1:
            if ves_area_analysis =='y':
                ...
                new_filling=np.array(list(zip(x_labels_final,y_labels_final,min_distances)))
            ...
            for k in df_all.keys():
                if k=='Summary results':
                    ...
                    if ves_area_analysis =='y':
                        ...
                        df_ss=pd.DataFrame(columns=['x_values','y_values','Distance to nearest vesicle (nm)'])
