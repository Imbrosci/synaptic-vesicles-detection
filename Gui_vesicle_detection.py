...
class VesAnalysisGui(tk.Frame):
    ...
    def plot(self):
        ...
        df_data2=pd.DataFrame(tuple2_list,columns=['Experiment','Vesicles min distance'])

        fig=Figure(figsize=(10,20))
        ax1=fig.add_subplot(1,2,1)
        a=sns.swarmplot(x='Experiment',y='Vesicles count',data=df_data1,ax=ax1) 
        a.set_xlabel('Experiments name',fontsize=18)
        a.set_ylabel('Vesicles count',fontsize=18)
        a.tick_params(labelsize=18)
        ax2=fig.add_subplot(1,2,2)
        b=sns.swarmplot(x='Experiment',y='Vesicles min distance',data=df_data2,ax=ax2)        
        b.set_xlabel('Experiments name',fontsize=18)
        b.set_ylabel('Vesicles min distance (nm)',fontsize=18)
