...
The file model.pth and model_post-pth, containing the weights of the two trained classifiers, are too big to be download by git and therefore are replaced by a text file. In order to download them correctly, after having cloned-downloaded the repository, go to: 
...
Then, replace, in the directory synaptic-vesicles-detection/vesicles_analysis, the model.pth and model_post.pth files downloaded by git with the manually downloaded ones.  
...
4.	Provide, as requested, an experiment name and the dimension of the pixels in nanometer and choose if you want the algorithm to estimate the vesicles area by typing 'y' or 'n' (and not yes or not). If you type a different letter or a different text, the algorithm, by default, is not going to estimate the vesicles area. Be aware, that if you decide to include this analysis, the algorithm may take a bit longer to process images.  
